...
void processVideo(char* videoFilename, double _fps) {
    ...
    while( (char)keyboard != 'q' && (char)keyboard != 27 ){
        ...
        char tmp[256];
    	getcwd(tmp, 256);
        cv::imwrite(string(tmp) +"/background.jpg",imbsMT->getBgModel());
...
void processImages(char* firstFrameFilename) {	
    ...
    while( (char)keyboard != 'q' && (char)keyboard != 27 ){
    	...
    	getcwd(tmp, 256);
    	//cout<<tmp<<endl;
        cv::imwrite(string(tmp) +"/data/background.jpg",imbsMT->getBgModel());
        //cout<<"Ho salvato l'immagine " << string(tmp) <<"/data/background.jpg"<<endl;

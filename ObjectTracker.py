...
class ObjectTracker():
    ...
    def __init__(self, preparation_instance, yolo_weights_path, strong_sort_weights):
        self.device = torch.device(device=config.device)
        self.preparation_instance = preparation_instance
        self.model = torch.hub.load('yolov5','custom', path=yolo_weights_path,force_reload=True,source='local')
    ...
    def track(self, img):
        """
            ...
            Returns:
                * (dict): tracking result containing IDs, classes,
                ...
                boxes. 
        """ 
        #calibrated_image = self.preparation_instance.calibrateFrame(img)
        ...
        outputs = self.tracker.update(xywhs, confidences, classes, img)
    ...
    def _to_another_space(self, bboxes_o):
        ...
        for bbox in bboxes_o:
            ...
            t = np.matmul(config.homography_matrix, o)

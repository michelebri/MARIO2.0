...
class Homography():
    ...
    def normalize_points(self,points_virtual_pitch, points_real_pitch):
        ...
        # hom_imp = np.array([np.array([[each[0dete], [each[1]], [1.0]]) for each in imp])
    ...
    def _from_detection(self):
        ...
        background = self.src_copy
        dh, dw, _ = background.shape
        
        for index, row in config.detectionT.iterrows():
            label = row['name']
            if label in ['L', 'T']:
                xmin, ymin, xmax, ymax = row['xmin'], row['ymin'], row['xmax'], row['ymax']

                cor_x = int((xmin + xmax) * 0.5)
                cor_y = int((ymin + ymax) * 0.5)

...
class StrongSORT(object):
    ...
    def update(self, bbox_xywh, confidences, classes, ori_img):
        ...
        bbox_tlwh = self._xyxy_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], conf, features[i]) for i, conf in enumerate(
            confidences)]
        
        classes_array = np.array(classes) if not isinstance(classes, np.ndarray) else classes
        ...
        self.tracker.update(detections, classes_array, confidences_array)
    ...
    def _xywh_to_tlwh(bbox_xywh):
        if isinstance(bbox_xywh, (list, tuple)):
        ...
        for box in bbox_xywh:
            bbox_tlwh[:,0] = box[0]  
            bbox_tlwh[:,1] = box[1]  
            bbox_tlwh[2] = box[2] - box[0]  
            bbox_tlwh[3] = box[3] - box[1]  

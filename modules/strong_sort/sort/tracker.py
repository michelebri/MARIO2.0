...
class Tracker:
    ...
    def update(self, detections, classes, confidences):
        ...
        for t in self.tracks:
            if t.is_deleted():
                # Current tracking has been deleted. The ID is now available
                # for assigment.
                self.ids[str(t.track_id)] = True
    ...
    def _initiate_track(self, detection, class_id, conf):
        id = self._get_next_available_id(class_id)
        if id == -1: return

        self.tracks.append(Track(
            detection.to_xyah(), id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,
            detection.feature))

    def _get_next_available_id(self, class_id):
        if   class_id == 0:
